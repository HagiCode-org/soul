...
当前默认首页已经切换为 editor-style Soul Builder 工作台。页面直接承接素材浏览、当前草稿、实时预览、本地保存与导出流程。

## 当前首页能力

- Builder-first 默认首页：默认路由直接进入 Soul Builder 工作台
- 编辑器式首页外壳：中央工作区常驻，左侧拆为 `Catalog` 与 `表达方式` 两个独立抽屉，右侧保留草稿、来源与状态抽屉
...
- `src/components/builder/MaterialLibraryPanel.tsx` / `SavedDraftRail.tsx` / `SourceFootnote.tsx`：以抽屉模式复用原有 Builder 能力，其中 `MaterialLibraryPanel` 分别承载 `Catalog` 与 `表达方式` 两个选择抽屉
...
- `src/components/builder/`：素材库、预览区、草稿区、来源说明区
